...
void BMRM_Solver::set_TolAbs(double tolabs) {
    _TolAbs = tolabs;
}

void BMRM_Solver::set_BufSize(int sz) {
    _BufSize = sz;
}

void BMRM_Solver::set_lambda(double lambda) {
    _lambda = lambda;
}

void BMRM_Solver::set_cleanICP(bool _cleanICP) {
    cleanICP = _cleanICP;
}
void BMRM_Solver::set_cleanAfter(int _cleanAfter) {
    cleanAfter = _cleanAfter;
}
void BMRM_Solver::set_K(double k) {
    K = k;
}
void BMRM_Solver::set_Tmax(int tmax) {
    T_max = tmax;
}
void BMRM_Solver::set_cp_models(int _cp_models) {
    cp_models = _cp_models;
}
void BMRM_Solver::set_verbose(bool _verbose) {
    verbose = _verbose;
}
 ...
 */
inline int find_free_idx(bool *map, int size) { for(int i=0; i<size; ++i) if (map[i]) return i; return size+1; }
...
bmrm_return_value_T BMRM_Solver::svm_bmrm_solver(
                                    double*       W,
                                    double        TolRel,
                                    double        TolAbs,
                                    double        _lambda,
                                    ...
                                    bool             verbose)
{
    ...
    if (verbose)
        printf("%4d: tim=%.3lf, Fp=%lf, Fd=%lf, R=%lf\n",
               bmrm.nIter, difftime(tstop,tstart), bmrm.Fp, bmrm.Fd, R);
    ...
    while (bmrm.exitflag==0)
    {
        ...
        if (verbose)
            printf("%4d: tim=%.3lf, Fp=%lf, Fd=%lf, (Fp-Fd)=%lf, (Fp-Fd)/Fp=%lf, R=%lf, nCP=%d, nzA=%d, QPexitflag=%d\n",
                      bmrm.nIter, difftime(tstop,tstart), bmrm.Fp, bmrm.Fd, bmrm.Fp-bmrm.Fd,
...
static const double *get_col_2( int i){
    return( &H2[ BufSize*i ] );
}

bmrm_return_value_T BMRM_Solver::svm_ppbm_solver(
                                    double*      W,
                                    double       TolRel,
                                    double       TolAbs,
                                    double       _lambda,
                                    ...
                                    bool            verbose)
{
    ...
    if (verbose)
        printf("%4d: tim=%.3f, Fp=%f, Fd=%f, R=%f, K=%f\n",
                  ppbmrm.nIter, difftime(tstop, tstart), ppbmrm.Fp, ppbmrm.Fd, R, K);
    ...
    while (ppbmrm.exitflag==0)
    {
        ...
        if (tuneAlpha)
        {
            ...
            while(flag)
            {
                ...
                if (::sqrt(sq_norm_Wdiff) > K)
                {
                    ...
                    {
                        LIBBMRM_MEMCPY(beta, beta_good, (size_t)ppbmrm.nCP*sizeof(double));
                ...
                else
                {
                    if (alpha > 0)
                    {
                        ...
                        LIBBMRM_MEMCPY(beta_good, beta, (size_t)ppbmrm.nCP*sizeof(double));
        ...
        /* Verbose output */
        if (verbose)
            printf("%4d: tim=%.3lf, Fp=%lf, Fd=%lf, (Fp-Fd)=%lf, (Fp-Fd)/Fp=%lf, R=%lf, nCP=%d, nzA=%d, wdist=%lf, alpha=%lf, qp_cnt=%d, gamma=%lf, tuneAlpha=%d\n",
                      ppbmrm.nIter, difftime(tstop, tstart), ppbmrm.Fp, ppbmrm.Fd, ppbmrm.Fp-ppbmrm.Fd,
